...
def game3():
    workbook = openpyxl.load_workbook("raw/Результати Зимовий Мажор.xlsx")
    sheet = workbook.active
    for row in sheet.iter_rows(min_row=2, values_only=True):
        name = row[1]
        score = row[2]
...
def game9():
    workbook = openpyxl.load_workbook("raw/Літній Мажор результати.xlsx")
    sheet = workbook.active
    for row in sheet.iter_rows(min_row=2, values_only=True):
        name = row[0]
        score = row[8]
...
def game10():
    workbook = openpyxl.load_workbook("raw/ССМП-3 - Результати в рейтинг.xlsx")
    sheet = workbook.active
    for row in sheet.iter_rows(min_row=2, values_only=True):
        name = row[0]
        score = row[1]
...
def game11():
    workbook = openpyxl.load_workbook("raw/Канхвєта - Результати в рейтинг.xlsx")
    sheet = workbook.active
    for row in sheet.iter_rows(min_row=2, values_only=True):
        name = row[0]
        score = row[1]
...
def game12():
    workbook = openpyxl.load_workbook("raw/Фінал Сезону.xlsx")
    sheet = workbook.active
    for row in sheet.iter_rows(min_row=2, values_only=True):
        name = row[0]
        score = row[1]
